...
export default function AdminPanel() {
  ...
  const handleInvite = async () => {
    ...
    setInviteOrgId('');
    setInviteScopeState('');
    setTimeout(() => setInviteSuccess(false), 15000); // show for 15s so admin can copy creds
  };
            ...
            <div className="flex flex-wrap gap-3 items-end">
              ...
              {inviteSuccess && (
                <div className="text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg p-3 space-y-1">
                  ...
                  <p className="text-green-600">User should log in at gmt.bluesapps.com and change their password.</p>
                </div>
              )}
            </div>
          </div>
