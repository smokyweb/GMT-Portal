...
export default function BudgetTracker() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* Grant Selector â€” dropdown or card grid */}
       {apps.length > 0 && (
         <>
           {apps.length === 1 ? (
                   ...
                   {apps.map(a => (
                     <SelectItem key={a.id} value={a.id}>
                       <span className="flex items-center gap-2">
                         {a.application_number} â€” {a.project_title} <span className="text-xs opacity-60">({a.program_code})</span>
                       </span>
                     </SelectItem>
                   ))}
                 </SelectContent>
                   ...
                   return (
                     ...
                     >
                       <div className="flex items-start justify-between gap-2 mb-2">
                         ...
                         <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-primary/10 text-primary flex-shrink-0">{a.program_code || 'â€”'}</span>
                       </div>
                       <div className="space-y-1.5">
                         ...
                         {daysLeft !== null && (
                           <p className={`text-[10px] ${daysLeft < 60 ? 'text-amber-600 font-medium' : 'text-muted-foreground'}`}>
                             {daysLeft}d remaining Â· ends {formatDateShort(a.performance_end)}
                           </p>
                         )}
                       </div>
                       {isSelected && (
        ...
        <>
          ...
          {selectedApp && (
            <div className="bg-card border rounded-xl p-4 flex flex-wrap gap-6">
              <div>
                ...
                <p className="font-semibold">{selectedApp.application_number} â€” {selectedApp.project_title}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Program</p>
                <p className="font-semibold">{selectedApp.program_code || 'â€”'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-semibold">{selectedApp.performance_start} â€“ {selectedApp.performance_end}</p>
              </div>
            </div>
          )}
              ...
              {warnings.map(r => (
                <div key={r.id} className={`flex items-start gap-3 rounded-lg px-4 py-3 border text-sm
                  ...
                  <div>
                    ...
                    {r.over
                      ? ` is over budget â€” ${formatCurrency(r.actual)} spent of ${formatCurrency(r.amount_requested)} budgeted (${r.pct.toFixed(1)}%)`
                      : ` is ${r.pct.toFixed(1)}% spent â€” only ${formatCurrency(r.amount_requested - r.actual)} remaining`
                    }
                  </div>
                </div>
              ))}
          ...
          {lineLoading ? (
            ...
            <div className="bg-card border rounded-xl overflow-hidden">
              ...
              {/* Grand total row â€” only when not filtered */}
              {!categorySearch && (
